...
RSENSOR_COMB_COUNT = (RSENSOR_COMB_COUNT // 2) * 2  # Make sure its even
...
ZCANT_BEAM_MAIN_WIDTH = 6
ZCANT_BEAM_MAIN_LENGTH = 120
...
ZCLAMP_BEAM_WIDTH = 3.5
ZCLAMP_BEAM_LENGTH = 100
...
ZCLAMP_PFLEX_POS = (
    ZCLAMP_POS[0] + ZCLAMP_PFLEX_BEAM_LENGTH + 310,
    ZCLAMP_POS[1]
...
ZCLAMP_COMB_FINGER_WIDTH = 4
ZCLAMP_COMB_WIDTH_TOTAL = (
    ZCLAMP_COMB_COUNT * ZCLAMP_COMB_GAP
    + (ZCLAMP_COMB_COUNT + 1) * ZCLAMP_COMB_FINGER_WIDTH
)
ZCLAMP_COMB_OVERLAP = 10
ZCLAMP_COMB_END_CLEARANCE = 10
ZCLAMP_COMB_WIDTH = (
    ZCLAMP_COMB_OVERLAP + 2 * ZCLAMP_PFLEX_STROKE + ZCLAMP_COMB_END_CLEARANCE
)
ZCLAMP_COMB_STATOR_WIDTH = 30

ZCLAMP_COMB_BANK = 6
ZCLAMP_COMB_BANK_SPACING = 20
ZCLAMP_COMB_BANK_COLUMN_LENGTH = ZCLAMP_COMB_WIDTH_TOTAL + 10
...
ZCLAMP_LOCKER_BEAM1_LENGTH = 150
ZCLAMP_LOCKER_BEAM1_WIDTH = 2.5
ZCLAMP_LOCKER_BEAM2_LENGTH = 150
ZCLAMP_LOCKER_BEAM2_WIDTH = 2.5
...
ZCLAMP_LOCKER_STOPPER_SIZE = (60, 80)
ZCLAMP_LOCKER_STOPPER_CLEARANCE = 15

ZCLAMP_PECK_STOPPER_SIZE = (60, 40)

ZCLAMP_TEST_STIFFNESS = False  # For validation testing

Z_CANT_BEAM_SPEC = None

ZACTUATOR_BASE_SIZE = (1200, 150)
# ZACTUATOR_FAT_SIZE = (900, 800)
ZACTUATOR_BASE_POS = (1050, 1300)

ZACTUATOR_BLOCKS_OFFSET = [70, 80, 120, 130, 150]
ZACTUATOR_BLOCKS_LENGTH = [1100, 990, 810, 650, 450]
ZACTUATOR_BLOCKS_HEIGHT = [150, 200, 200, 150, 150]
ZACTUATOR_BLOCKS_SUBDIV = 2
...
ZCANT_BEAM_STOPPER_OUTER_WIDTH = 80
...
def z_clamp() -> gf.Component:
    ...
    y4 = y3 - ZCLAMP_COMB_STATOR_WIDTH
    ...
    stor = gf.components.rectangle(
        size=(x4 - x3, ZCLAMP_COMB_STATOR_WIDTH),
        centered=False,
    ...
    (
        ...
        << gf.components.rectangle(
            size=(ZCLAMP_COMB_STATOR_WIDTH, -dy + ZCLAMP_COMB_STATOR_WIDTH),
            centered=False,
            layer=LAYERS.DEVICE_P3,
        )
    ).move((x4, y4 + dy))
    ...
    n_cap = ZCLAMP_COMB_BANK * ZCLAMP_COMB_COUNT
    ...
    a = (ZCLAMP_LOCKER_BEAM_SEPARATION + ZCLAMP_LOCKER_BEAM1_WIDTH) / 2
    len_actual = (
        ZCLAMP_LOCKER_BEAM1_LENGTH
        - ZCLAMP_LOCKER_RING_OUTER_RADIUS
        + np.sqrt(ZCLAMP_LOCKER_RING_OUTER_RADIUS**2 - a**2)
    )
    len_actual2 = (
        ZCLAMP_LOCKER_BEAM2_LENGTH
        - ZCLAMP_LOCKER_RING_OUTER_RADIUS
        + np.sqrt(ZCLAMP_LOCKER_RING_OUTER_RADIUS**2 - a**2)
    )
    beamH = gf.components.rectangle(
        size=(ZCLAMP_LOCKER_BEAM1_LENGTH, ZCLAMP_LOCKER_BEAM1_WIDTH),
        centered=False,
        layer=LAYERS.DEVICE_P3,
    )
    ...
    ring1_pos = (
        ...
        yr0 + ZCLAMP_LOCKER_BEAM1_WIDTH / 2,
    )
    (
        c
        << gl.basic.ring(
    ...
    (xr1, yr1) = (
        ring2_pos[0] - ZCLAMP_LOCKER_BEAM2_WIDTH / 2,
        ring2_pos[1]
        - ZCLAMP_LOCKER_RING_OUTER_RADIUS
        - len_actual2
        + gl.utils.sagitta_offset_safe(
            radius=ZCLAMP_LOCKER_RING_OUTER_RADIUS,
            chord=ZCLAMP_LOCKER_BEAM2_WIDTH,
            angle_resolution=ANGLE_RESOLUTION,
        ),
    )
    beamV = gf.components.rectangle(
        size=(ZCLAMP_LOCKER_BEAM2_WIDTH, ZCLAMP_LOCKER_BEAM2_LENGTH),
        centered=False,
        layer=LAYERS.DEVICE_P3,
    )
    ...
    # Generate stopper at pecker
    (xr4, yr4) = (
        xr0,
        yr0 - 2 * ZCLAMP_LOCKER_RING_OUTER_RADIUS - ZCLAMP_PECK_STOPPER_SIZE[1],
    )
    (
        c
        << gl.basic.rectangle(
            size=ZCLAMP_PECK_STOPPER_SIZE,
            geometry_layer=LAYERS.DEVICE_P3,
            centered=False,
            release_spec=RELEASE_SPEC,
        )
    ...
    (
        ...
        << gf.components.rectangle(
            size=(
                ring1_pos[0] + ZCLAMP_LOCKER_STOPPER_SIZE[0] / 2 - xr5,
                ZCLAMP_PECK_STOPPER_SIZE[1],
            ),
            layer=LAYERS.DEVICE_P3,
            centered=False,
        )
